...
static void resize(vector *ptr_vect, int n, void *value)
{
    void **arr = realloc(ptr_vect->arr, sizeof(void *) * n);
    for (int i = 0; i < n; ++i)
    {
        arr[i] = value;
    }
...
{
    void **arr = realloc(ptr_vect->arr, sizeof(void *) * ptr_vect->sz);
    ptr_vect->cap = ptr_vect->sz;
    ptr_vect->arr = arr;
}

//to set a vector with a specific size where each element is equal to the parameter value
static void assign(vector *ptr_vect, int sz, void *value)
{
    if (sz > ptr_vect->sz)
    {
        void **arr = realloc(ptr_vect->arr, sizeof(void *) * sz);
        for (int i = 0; i < sz; ++i)
        {
            arr[i] = value;
        }
...
static void push_back(vector *ptr_vect, void *g)
{
    ...
    {
        void *arr = realloc(ptr_vect->arr, sizeof(void *) * (ptr_vect->sz + 1));
        ptr_vect->arr = arr;
        ptr_vect->sz += 1;
        ptr_vect->cap += 1;
...
static void insert(vector *ptr_vect, iterator_vector ptr_iter, void *val)
{
    void **temp_arr = malloc(sizeof(void *) * (ptr_vect->sz + 1));
...
static void **at(vector *ptr_vect, int position)
{
...
static void **front(vector *ptr_vect)
{
    if (ptr_vect->sz == 0)
    {
...
static void **back(vector *ptr_vect)
{
    if (ptr_vect->sz == 0)
    {
        fprintf(stderr, "Error: Access to Empty list \n");
...
static void **data(vector *ptr_vect)
{
    return ptr_vect->arr;
}

//adds an element to the position specified by the iterator passed as an arguement by doing in-place.
static iterator_vector emplace(vector *ptr_vect, iterator_vector ptr_iter, void *element)
{
    if (ptr_vect->sz < ptr_vect->cap)
    {
        int position = ptr_iter.index;
...
static void emplace_back(vector *ptr_vect, void *element)
{
    if (ptr_vect->sz < ptr_vect->cap)
    {
    ...
    {
        ptr_vect->arr = realloc(ptr_vect->arr, sizeof(void *) * (ptr_vect->sz + 1));
        int position = ptr_vect->sz;
        ptr_vect->arr[position] = element;
        ptr_vect->sz += 1;
        ptr_vect->cap += 1;
